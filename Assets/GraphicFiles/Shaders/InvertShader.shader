	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (pixel i) : SV_Target
			{
			    ...
			    float2 r2 = dot(coordinate, coordinate);
			    ...
			    float2 coordinateDistribution = float2(cos(a), sin(a))/r;

			    coordinateDistribution += 10.0 * cos(float2(0.6,0.3) + float2(0.1, 0.13) * TIME);

			    col = r * tex2D(_TextureChannel0, coordinateDistribution * 0.25).xyz;
